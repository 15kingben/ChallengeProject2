...
GLfloat lightpositionst[4*numLights];
void display() 
{
  ...
  if (numused) {
	...
	for(int i = 0; i < numused; i++){
		transformvec(&lightposn[4*i], &lightpositionst[i*4]);


	}

	glUniform4fv(lightpos,numused,lightpositionst);
  ...
  //transf = (mv)*glm::transpose(Transform::translate(tx,ty,0.0))*glm::transpose(Transform::scale(sx,sy,1.0));
  transf = (mv)*Transform::translate(tx,ty,0.0)*glm::transpose(Transform::scale(sx,sy,1.0));
